...
for ran in [0]:
    ...
    for row in rows:
        ...
        soap_np = soap.create(atoms)
        soap_lst = list(soap_np)
        soap_arr = [[0 for x in range(3696)] for y in range(30)]
        for i, j in enumerate(soap_lst):
            soap_arr[i] = j

        soap_data = []
        for i in range(len(soap_arr)):
            for j in range(len(soap_arr[0])):
                soap_data.append(soap_arr[i][j])
        data.append(soap_data)
